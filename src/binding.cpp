	...
	auto &solver = py::class_<polyfem::State>(m, "Solver")
		...
		.def("load_mesh_from_path_and_tags", [](polyfem::State &s, const std::string &path, const std::string &bc_tag) {
			...
			s.load_mesh();
			 ...
			 "Loads a mesh and bc_tags from path", py::arg("path"), py::arg("bc_tag_path"))
		...
		.def("set_mesh", [](polyfem::State &s, const Eigen::MatrixXd &V, const Eigen::MatrixXi &F) {
			...
			s.load_mesh();
			 ...
			 "Loads a mesh from vertices and connectivity", py::arg("vertices"), py::arg("connectivity"))

		.def("set_high_order_mesh", [](polyfem::State &s, const Eigen::MatrixXd &V, const Eigen::MatrixXi &F, const Eigen::MatrixXd &nodes_pos, const std::vector<std::vector<int>> &nodes_indices) {
			init_globals(s);
			py::scoped_ostream_redirect output;
			...
			s.load_mesh();
			 ...
			 "Loads an high order mesh from vertices, connectivity, nodes, and node indices mapping element to nodes", py::arg("vertices"), py::arg("connectivity"), py::arg("nodes_pos"), py::arg("nodes_indices"))
